...
from mopidy_spotify.oauth import flow, state, store
...
def web(
    *,
    ...
    auth_flow = flow.AuthFlow(config, auth_state_path, storage=storage)
...
def _web_status(config: Config) -> str:
    ...
    if isinstance(auth_state, state.PkceAuthorizedAuthState):
        return "authorized (PKCE)"
    if (
        isinstance(auth_state, state.PermanentErrorAuthState)
        and auth_state.mode == "pkce"
    ):
