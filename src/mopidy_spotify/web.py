...
type RefreshContext = tuple[
    auth_state.AuthStateSnapshot | None,
...
class SpotifyOAuthClient(OAuthClient):
    ...
    def __init__(
        ...
        *,
        ...
        if auth_state_path is None:
            ...
            self._auth_state_store = auth_state.AuthStateStore(auth_state_path)
    ...
    def _load_auth_state(self) -> auth_state.AuthStateSnapshot | None:
        ...
        try:
            ...
            auth_state.InvalidAuthStateError,
            auth_state.AuthStateStoreError,
    ...
    def _save_auth_state(
        self,
        expected: auth_state.AuthStateSnapshot | None,
        state: auth_state.AuthState,
    ) -> None:
        if self._auth_state_store is None:
            return
        try:
            saved = self._auth_state_store.save_if_current(expected, state)
        except (
            auth_state.InvalidAuthStateError,
            auth_state.AuthStateStoreError,
            secrets.SecretStoreError,
    ...
    def _handle_token_refresh_success(
        ...
        *,
        ...
        self._save_auth_state(
            snapshot,
            provider.state_after_success(response, state),
        )
    ...
    def _handle_token_refresh_error(
        ...
        *,
        ...
        snapshot, provider = refresh_context
        state = snapshot.state if snapshot is not None else None
        next_state = provider.state_after_error(response, state, status_code)
