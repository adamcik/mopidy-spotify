...
def test_get_rotates_keyring_refresh_token(
    ...
    keyring.values["mopidy-spotify", "original-id"] = "refresh-token-1"
    path.write_text(
        json.dumps(
            {
                "version": 1,
    ...
    with mock.patch.object(secrets.importlib, "import_module", return_value=keyring):
        assert client.token() == "access-token"

    manifest = json.loads(path.read_text())
    username = manifest["refresh_token"]["username"]
    assert username != "original-id"
    assert keyring.values == {("mopidy-spotify", username): "refresh-token-2"}
...
def test_pkce_refresh_success_does_not_overwrite_newer_authorization(
    ...
    client, refresh_token_path = refresh_token_oauth_client
    store = auth_state.AuthStateStore(refresh_token_path)
    replacement = auth_state.PkceAuthorizedAuthState(
        refresh_token=SecretStr("refresh-token-new")
    )

    def authorize_during_refresh(_: requests.PreparedRequest):
        store.authorize(replacement.refresh_token)
        return (
            200,
            {},
            json.dumps(
...
def test_pkce_refresh_does_not_cache_token_when_persistence_fails(
    ...
    with mock.patch.object(
        ...
        "save_if_current",
        side_effect=secrets.SecretStoreError,
    ):
        assert client.token() is None
...
def test_get_clears_expired_refresh_token_and_fails_fast(
    ...
    assert "Run `mopidy spotify auth` to reauthorize" in caplog.text
...
def test_get_fails_fast_when_auth_json_is_invalid(
    ...
    assert "Run `mopidy spotify auth` to replace it" in caplog.text
