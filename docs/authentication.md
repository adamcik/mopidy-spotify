...
Before persisting a refresh result, `oauth.store.Store.save_if_current()` locks
...
| `oauth/state.py`       | Pure manifest, descriptor, and runtime-state models.                                      |
| `oauth/store.py`       | Manifest I/O, locking, keyring rotation, and compare-and-save transitions.                |
