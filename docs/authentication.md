...
`auth.json` contains a versioned, discriminated authorization payload.
...
The payload stores PKCE refresh tokens, but never bridge credentials or access
...
1. A valid `pkce/authorized` payload selects the PKCE provider.
2. Missing or cleared PKCE state permits bridge fallback.
3. A `pkce/permanent_error` payload fails closed until reauthorization.
4. A `bridge/permanent_error` payload does not block the bridge. This allows
...
| Secret storage      | Loading, saving, and clearing one refresh token in a chosen backend.    | Manifest or OAuth state.        |
| Atomic replacement  | Durable replacement of one complete byte sequence.                      | Conflict or transition policy.  |

The manifest is a versioned JSON document. Every payload records its mode and
...
`AuthStateStore` owns the manifest schema and always reads and writes
`<core/data_dir>/spotify/auth.json`. It resolves the descriptor to inline
content or constructs a `SecretStore` for the named external backend. A
`SecretStore` stores only the refresh token; it never stores the complete
manifest and does not interpret OAuth state.
...
chosen explicitly when authorization is created. `mopidy spotify auth` uses
inline storage by default; `--storage keyring` selects keyring storage when the
optional `keyring` dependency is installed. The descriptor in the manifest
remains authoritative until authorization is replaced.

`SecretStr` prevents accidental disclosure through Python representations and
validation diagnostics; it does not encrypt an inline serialized token.
...
Before persisting a refresh result, `FileAuthStateStore.save_if_current()` locks
the state and compares it with the snapshot used to build the request. If the
state changed, the stale result is rejected and its access token is not
installed.
...
then compare-and-saves a manifest that references the new address. If the state
comparison fails, the candidate entry can be removed without affecting current
...
it protects a semantic compare-and-save transition. Atomic replacement has the
...
| Invalid or unreadable auth state     | Unchanged                              | Replace it with `mopidy spotify auth`.                          |
...
| Permanent PKCE error                 | `pkce/permanent_error`                 | Run `mopidy spotify auth` again.                                |
...
| ---------------------- | ----------------------------------------------------------------------------------------- |
| `auth_flow.py`         | Interactive PKCE challenge, callback validation, code exchange, and initial persistence.  |
| `auth_state.py`        | Manifest schema, token descriptors, versioning, locking, and compare-and-save.            |
| `refresh_providers.py` | Bridge and PKCE request selection and state-transition policy.                            |
...
| `secrets`              | Resolution and I/O for refresh tokens held in external secret backends.                   |
